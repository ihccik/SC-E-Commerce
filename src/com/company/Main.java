...
//                            GiftCard gb=findGiftCardBalance(customer.getId());
                ...
                case 9:
                    System.exit(1);
                    break;
                case 10: //Transfer account balance to gift card balance
                    transferBetweenBalances(customer);
                    break;
            }
    ...
    private static void transferBetweenBalances(Customer customer) {

        double accountBalance = StaticConstants.CUSTOMER_BALANCE_LIST.stream().filter(balance -> balance.getCustomerId() == customer.getId())
                .findFirst().get().getBalance();
        System.out.println("Account Balance: " + accountBalance );

        double giftBalance = StaticConstants.GIFT_CARD_BALANCE_LIST.stream().filter(balance -> balance.getCustomerId() == customer.getId())
                .findFirst().get().getBalance();

        System.out.println("Gift Card Balance: " + giftBalance);

        Scanner scan = new Scanner(System.in);
        System.out.println("How much would you like to transfer from your balance");
        ...
        do{
            ...
            if(accountBalance >= transferAmount){
                ...
                System.err.println("You do not have enough balance, please type another amount.");
            }
        }while(true);
    ...
    private static String[] prepareMenuOptions() {
        return new String[]{"List Categories", "List Products", "List Discount", "See Balance",
                ...
                "Place an order", "See Cart", "See order details", "See your address", "Close App", "Transfer account balance to gift card balance"};
    }
